    ...
    processData: false,
        ...
        success: function(data){
            ...
            if(response.error==0)
            {
                ...
                var target = jQuery('#app_photoimg').attr('target');
                var input  = jQuery('#app_photoimg').attr('input');
...
function custom_sendFileToServer(formData,status,feature_img_progress,target)
{
    ...
    var jqXHR=$.ajax({
        ...
        success: function(data){
            ...
            {
                var base_url  = '<?php echo base_url();?>';
                var target = jQuery('#'+target_obj).attr('target');
                var input  = jQuery('#'+target_obj).attr('input');
//                var target = jQuery('#app_photoimg').attr('target');
//                var input  = jQuery('#app_photoimg').attr('input');
...
function custom_handleFileUpload(files,obj, target)
{
   for (var i = 0; i < files.length; i++)
   {
        var fd = new FormData();
        ...
        var feature_img_progress = 0;
        var status = new createStatusbar(obj); //Using this we can set progress.
        custom_sendFileToServer(fd,status,feature_img_progress,target);
